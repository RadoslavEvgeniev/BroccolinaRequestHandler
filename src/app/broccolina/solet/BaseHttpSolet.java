...
public abstract class BaseHttpSolet implements HttpSolet {
    ...
    protected void doGet(HttpSoletRequest request, HttpSoletResponse response) {
        ...
        response.setContent(("<h1>[ERROR] GET " + request.getRequestUrl() + "</h1><br/><h3>[MESSAGE] The page or functionality you are looking for is not found. </h3>").getBytes());
    }

    protected void doPost(HttpSoletRequest request, HttpSoletResponse response) {
        this.configureNotFound(request, response);

        response.setContent(("<h1>[ERROR] POST " + request.getRequestUrl() + "</h1><br/><h3>[MESSAGE] The page or functionality you are looking for is not found. </h3>").getBytes());
    }

    protected void doPut(HttpSoletRequest request, HttpSoletResponse response) {
        this.configureNotFound(request, response);
        response.setContent(("<h1>[ERROR] PUT " + request.getRequestUrl() + "</h1><br/><h3>[MESSAGE] The page or functionality you are looking for is not found. </h3>").getBytes());
    }

    protected void doDelete(HttpSoletRequest request, HttpSoletResponse response) {
        this.configureNotFound(request, response);
        response.setContent(("<h1>[ERROR] DELETE " + request.getRequestUrl() + "</h1><br/><h3>[MESSAGE] The page or functionality you are looking for is not found. </h3>").getBytes());
    }
